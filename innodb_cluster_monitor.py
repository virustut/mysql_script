...
secondaries = members_df[members_df["MEMBER_ROLE"] == "SECONDARY"]["MEMBER_HOST"].tolist()

if secondaries:
    target = st.selectbox("Select new PRIMARY", secondaries)
    if st.button("⚡ Trigger Failover"):
        if manual_failover(target):
            st.success("Failover triggered")
            time.sleep(2)
            st.experimental_rerun()
else:
    st.info("No SECONDARY nodes available")
